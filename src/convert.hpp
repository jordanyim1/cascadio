...
/// Transcode STEP or IGES to GLB ( the binary format of glTF )
static int to_glb(char *input_path, char *output_path, FileType file_type,
  ...
  case IGES: {
    ...
    // parse the IGES file
    if (IFSelect_RetDone != igsReader.ReadFile((Standard_CString)input_path)) {
    ...
    if (!igsReader.Transfer(doc)) {
      std::cerr << "Error: Failed to convert IGES file \"" << input_path << "\" !"
                << std::endl;
      doc->Close();
      return 1;
    }
...
/// Transcode STEP or IGES to OBJ ( and if applicable a MTL sidecar file )
static int to_obj(char *input_path, char *output_path, FileType file_type,
                  Standard_Real tol_linear, Standard_Real tol_angle,
                  Standard_Boolean tol_relative,
                  Standard_Boolean use_parallel,
  ...
  case IGES: {
    ...
    // Disabling color-mode will result in no sidecar MTL file being exported.
    igsReader.SetColorMode(use_colors);
    igsReader.SetNameMode(true);
    igsReader.SetLayerMode(true);
