...
MICROSTRIP_IGES_PATH = MODELS_DIR / "microstrip.igs"
TOL_LINEAR = 0.1
TOL_ANGULAR = 0.5
...
def test_convert_step_to_glb():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        cascadio.to_glb(
            step_path.as_posix(),
            glb_path.as_posix(),
            file_type=cascadio.FileType.STEP,
...
def test_convert_iges_to_glb():
    iges_path = MICROSTRIP_IGES_PATH

    with tempfile.TemporaryDirectory() as temp_dir:
        glb_path = Path(temp_dir) / iges_path.with_suffix(".glb").name
        cascadio.to_glb(
            iges_path.as_posix(),
            glb_path.as_posix(),
            file_type=cascadio.FileType.IGES,
            tol_linear=TOL_LINEAR,
            tol_angular=TOL_ANGULAR,
        )
        scene = trimesh.load(glb_path, merge_primitives=True)

    # Verify the GLB was created and has geometry
    assert len(scene.geometry) >= 1
    # Verify the mesh has valid geometry (vertices and faces)
    # IGES files often contain surfaces, not solids, so we check for mesh data
    for geom in scene.geometry.values():
        assert len(geom.vertices) > 0
        assert len(geom.faces) > 0


def test_convert_iges_to_obj():
    iges_path = MICROSTRIP_IGES_PATH

    with tempfile.TemporaryDirectory() as temp_dir:
        obj_path = Path(temp_dir) / iges_path.with_suffix(".obj").name
        cascadio.to_obj(
            iges_path.as_posix(),
            obj_path.as_posix(),
            file_type=cascadio.FileType.IGES,
    ...
    # Verify the OBJ was created and has valid geometry
    # IGES files often contain surfaces, not solids, so we check for mesh data
